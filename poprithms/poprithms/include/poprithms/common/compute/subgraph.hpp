...
/**
 * A tensor is a thin wrapper around (1) a sub-graph id and (2) a graph, which
 * allows for  sub-graph-centric code where methods are called on
 * sub-SubGraphs instead of on graphs.
 * */

class SubGraph {

public:
  SubGraph(const Tensor &t) : SubGraph(t.subGraphId(), t.graph()) {}

  SubGraph(SubGraphId id, Graph &graph) : id_(id), pGraph_(&graph) {}
   ...
   * */
  Tensor constant(const HostTensor &v, DeviceId d);

  /**
   * Create a scalar constant tensor, of type #t and value #v, on device #d.
   * */
  Tensor constant(DType t, double v, DeviceId d) {
   ...
   * */
  Tensor variable(DType t, const Shape &s, DeviceId d);

  /**
   * Create a variable tensor in this sub-graph with tensor information #info.
   * */
  Tensor variable(const TensorInfo &info) {
   ...
   * */
  Tensor hostVariable(DType t, const Shape &s) {
   ...
   * */
  Tensor rootIpuFloat32Variable(const Shape &s) {
   ...
   * */
  Tensor hostFloat64Variable(const Shape &s) {
   ...
   * */
  Tensor hostFloat32Variable(const Shape &s) {
   ...
   * */
  Tensor hostInt32Variable(const Shape &s) {
   ...
   * */
  Tensor float32Constant(float v, DeviceId dId) {
   ...
   * */
  Tensors variables(DType dtype, const Shapes &shapes, DeviceId deviId);
   ...
   * #likeThese.
   * */
  Tensors variablesLike(const Tensors &likeThese);

  /**
   * \return All tensors in this sub-graph.
   * */
  Tensors tensors() {
    return Tensor::tensors(graph().tensorIds(id()), graph());
  }
