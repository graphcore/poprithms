...
public:
  ...
  // A Graph can be generated by merging all linked Ops in this Graph into
  // single Ops. The method below does this. The returned tuple
  // consists of (1) the reduced Graph, containing merged Ops and (2) a
  // mapping from the Ops in the reduced (child) Graph to Ops in this (the
  // parent) Graph
  using ParentGraphOps = std::vector<std::vector<OpAddress>>;
  using LinkMerged     = std::tuple<Graph, ParentGraphOps>;
  LinkMerged getLinkMerged() const;
...
public:
  // Combine all forward and backward links to form isolated chains
