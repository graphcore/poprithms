...
poprithms::schedule::shift::Graph getBranchDoublingGraph(uint64_t nBranches,
                                                         uint64_t offset) {
...
void assertGlobalMinimumBranchDoubling(const ScheduledGraph &sg,
                                       int nBranches,
                                       int offset) {
  auto branchLengths = getBranchLengths(sg.getGraph());
