...
void test(
  ...
  auto pmo = PathMatrixOptimizations::allOff();
  pmo.withConstrainWeightSeparatedGroups().withMaxIterations(1);
  // PathMatrixOptimizations pmo({0, 1});
  g.initialize(KahnTieBreaker::RANDOM, 1011, pmo);
  ...
  for (uint64_t i : expectSameOuts) {
    if (g.getOp(i).getOuts() != gBefore.getOp(i).getOuts()) {
  ...
  auto outs      = g.getOp(from).getOuts();
