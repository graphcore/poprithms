...
void test0() {
 ...
 std::vector<double> allocWeights = 
  ...
  auto gCopy = g;
  gCopy.initialize(
      KahnTieBreaker::RANDOM, 1011, PathMatrixOptimizations::allOff());
  if (!gCopy.getLinkChains().empty()) {
    throw error(
        "With all path matrix optimizations off, expected no link chains");
  }

  const auto pom = PathMatrixOptimizations::allOff().withLinkTightDrops();
