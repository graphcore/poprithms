...
int main() {
  ...
  if (x0.size() != 3) {
    throw poprithms::test::error(
        "Expected output of getExtremumStatuses to 1-1 with input");
  }

  if (x0[0] != std::tuple{IsFirst::Yes, IsFinal::No}) {
    throw poprithms::test::error("Expected \"0\" to be first and not final");
  }
  if (x0[1] != std::tuple{IsFirst::No, IsFinal::No}) {
    throw poprithms::test::error("Expected \"1\" to be in middle");
  }
  if (x0[2] != std::tuple{IsFirst::No, IsFinal::Yes}) {
    throw poprithms::test::error("Expected \"0\" to be final and not first");
  }

  x0 = em.getExtremumStatuses({3});
  if (x0[0] != std::tuple{IsFirst::Yes, IsFinal::Yes}) {
    throw poprithms::test::error(
        "Exect getExtremumStatuses, called on a singleton, to "
  ...
  for (const auto &x : x0) {
    if (x != std::tuple{IsFirst::Maybe, IsFinal::Maybe}) {
      throw poprithms::test::error(
          "Expected {Maybe, Maybe} for all diamond edges");
    }
  }
  ...
  for (uint64_t i = 1; i < 4; ++i) {
    if (x0[i] != std::tuple{IsFirst::Maybe, IsFinal::No}) {
      throw poprithms::test::error(
          "Expected {Maybe, No} for diamond edge with peak");
    }
  }
