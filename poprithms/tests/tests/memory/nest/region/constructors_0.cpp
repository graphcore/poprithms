...
int main() {
  ...
  if (!r4.equivalent(r1)) {
    throw error("Stripe constructor gives unexpected Region");
  }
  if (r4.equivalent(r0)) {
    throw error("r4 compared equvalent to r0, in constructro test");
  }

  const auto r5 = Region::fromBounds({10}, {3}, {6});
  if (!r5.equivalent({{10}, {{{{6 - 3, 10 - (6 - 3), 3}}}}})) {
    std::cout << r5 << std::endl;
    throw error("Incorrect region constructed by fromBounds");
  }
}
