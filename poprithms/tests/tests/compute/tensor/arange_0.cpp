...
void test0() {
  ...
  for (int i = 10; i > -25; i -= 7) {
    if (vals[index++] != i) {
      throw error("Error in first arange test, expected 10, 3, -4...");
    }
  }
  ...
  if (fRange.nelms_u64() != 3) {
    throw error("Should be exactly 3 elements in this float16 range (-100, "
                "-1.5, 97)");
  }
  const auto fVals = fRange.getFloat64Vector();
  if (fVals[1] + 1.5 != 0.0) {
    throw error("Incorrect value (-100 + 98.5 = -1.5)");
  }
}
} // namespace
